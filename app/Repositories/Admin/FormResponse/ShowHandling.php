...
/**
 * Show Form Response detail data.
 */
class ShowHandling
{
  protected $request;
  ...
  {
    $this->data = FormResponse::where('id', $this->id)->first();

    if (!$this->data) {
      throw new \Exception('Form response not found!', 404);
    }
  }
  ...
  public function handle()
  {
    ...
    $this->alumni = Alumni::with(['batch', 'major'])->where('id', $this->data->alumni_id)->first();
    ...
    foreach ($data->sections as $key2 => $section) {
      foreach ($section->questions as $key3 => $question) {
        ...
        $answer = FormResponseAnswer::select([
          ...
          ->where('form_responses.form_id', $this->data->form_id)
          ->where('form_responses.alumni_id', $this->data->alumni_id)
